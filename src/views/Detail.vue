<template>
  <div class="detail">
    ...
    <!-- 商品加入购物车提示 显示文本和动画样式都动态获取 -->
      <!-- 动态切换class显示show动画和dispear动画，但dispear未实现 -->
    <Toast
      :message="message"
      :class="[activeCls ? showCls : dispearCls]" 
      v-show="show"
    />
...
import {
  ...
  Detail,
  getRecommend,
} from "network/detailRequest"; //引入请求方法，两个整合类
...
export default {
  ...
  data() {
    return {
      swiperOptions: {
        ...
        autoplay: {
          delay: 2000,
          disableOnInteraction: false,  //手动滑动轮播图后轮播是否停止
        },
      },
      id: null, //商品id
      list: [], //轮播图
  ...
  created() {
    ...
    getDetailGoods(this.id).then((res) => {
      //通过保存的id 获取详情页数据
      console.log(res.data);
      const data = res.data.result;
      this.list = data.itemInfo.topImages; //获取详情页轮播图
      this.baseGoodsInfo = new Goods(
        data.itemInfo,
        data.columns,
        data.shopInfo
      ); //创建商品基础信息实例
  ...
  methods: {
    ...
    pageScroll() {
      ...
      switch (true) {
        ...
        case this.pageYposition < this.$refs.paramInfo.$el.offsetTop:
          ...
          break;
        case this.pageYposition >= this.$refs.paramInfo.$el.offsetTop &&
          this.pageYposition <= this.$refs.rate.$el.offsetTop:
          this.currentIndex = 1;
          break;
        case this.pageYposition >= this.$refs.rate.$el.offsetTop &&
          this.pageYposition <= this.$refs.recommend.$el.offsetTop:
          this.currentIndex = 2;
          break;
    ...
    addToCart() {
      ...
      this.$store.dispatch("getAddGoods", product).then((res) => {
        //getAddGoods方法包装了一层promise,这里就可以回调then
        console.log(res);
        this.show = true; //显示Toast
        this.activeCls = true; //绑定show-toast动画
        this.message = res;
        setTimeout(() => {
          this.show = false;  //toast消失
          this.activeCls = false; //绑定dipear动画 ***未实现
          this.message = "";
        }, 2000);
      });
    },
